...
class LibMatchDescriptor:
    ...
    def __init__(self, bv: BinaryView, filename: str, banned_names: set = ("$d", "$t")):
        ...
        for fun in self.bv.functions:
            ...
            self.normalized_functions[fun.start] = NormalizedFunction(self.bv, fun)
            for node in fun.create_graph().nodes:
                block = node.basic_block
                try:
                    # TODO high: fix project parameter
                    self.normalized_blocks[(fun.start, block.start)] = NormalizedBlock(
                        self.bv, b, self.normalized_functions[fun.start]
                    )
        ...
        for faddr in self.function_manager:
            ...
            if f.is_plt or f.is_simprocedure or not f.name or f.name in banned_names or self.is_trivial(proj, f):
    ...
    def _build_callgraph(self, bv: BinaryView):
        """Build callgraph from BinaryNinja function call references"""
        call_graph = {}
        for function in bv.functions:
            call_graph[function.name] = {
                "callers": [caller.name for caller in function.callers],
                "callees": [callee.name for callee in function.callees],
            }
        return call_graph
    ...
    def _compute_function_attributes(self) -> dict:
        ...
        for function_addr in self.cfg.kb.functions:
            ...
            if (
                self.cfg.kb.functions.function(function_addr) is None
                or self.cfg.kb.functions.function(function_addr).is_syscall
            ):
                continue
    ...
    def symbol_for_addr(self, addr):
        ...
        for s in self.loader.extern_object.symbols:
            if s.rebased_addr == addr:
                # TODO high: restore whatever was here
                pass

    # Creation and Serialization
    # @staticmethod
    # def make_signature_dump(filename: str) -> Path:
    #     lmd = LibMatchDescriptor(filename)
