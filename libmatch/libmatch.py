from .libmatch_database import LibMatchDatabase
...
class LibMatch():
    ...
    @classmethod
    def _first_order_heuristic(cls, lib_attrs, bin_attrs):
        """
    ...
    def _postprocess_second_order_matches(self) -> Dict[int, List]:
        # TODO low: better description / comment
        # Gather the matches based on the functions in the original binary:
        matches = defaultdict(list)  # TODO medium: replace defaultdict with something more concise
        ...
        for lib_name, match_lmd, match_diff in matches:
            ...
            for possible_targ_callees, lib_callee in zip(target_callees, lib_callees):
                ...
                try:
                    ...
                    log_error("Hmm, something is wrong %#08x %#08x %s" % (f_addr, lib_callee, match_lmd.filename))
                    return
                ...
                if not self.binary_lmd.loader.main_object.contains_addr(targ_callee):
                    ...
                    if not guessed_sym:
                        log_warn(
                            ...
                            "Couldn't figure out what {lib_callee:x} is, called by func {lib_func.addr}"
                        )
                        continue
                    guessed_name = guessed_sym.name
                    new_matches = []
